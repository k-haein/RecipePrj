...
<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath}/resources/css/recipe_write.css" media="all" />
...
function addMaterial(group_idx, init_json, prev_step){
    ...
    str += '<input type="text" name="mate_name" class="step'+group_idx+'_'+step+'" id="mate_name"  style="width:330px;">';
    str += '<input type="text" name="mate_cnt" class="cok_material_amt_'+group_idx+'_'+step+'" id="mate_cnt"  style="width:330px;">';
    ...
    if (typeof init_json !== 'undefined' && init_json !== null && init_json['mat_nm_material']) {
        $("#divMaterialArea_"+group_idx+" class=step" + group_idx + "_" + step ).val(init_json['mat_nm_material']);
       
    } else {
   			
      $("#divMaterialArea_"+group_idx+" class=step" + group_idx + "_" + step ).attr('placeholder','예) '+_MATERIAL_SAMPLE[(step-1)%_MATERIAL_SAMPLE.length]['mat_nm_material']);
...
function addRecipeMaterial(group_idx, init_json, prev_step){
    ...
    var w1 = ($("#cok_reg_type").val() == 'edit') ? 180 : 610;
	var w2 = ($("#cok_reg_type").val() == 'edit') ? 180 : 150;
    var str = '';

	/* str += '<li id="rcMaterial_'+group_idx+'_'+step+'"><a href="#" class="btn-lineup"></a>'; */
    str += '<input type="text" name="step" id="step'+group_idx+'_'+step+'" class="form-control" style="width:'+w1+'px; height:'+w2+'px;"  placeholder="Step'+stepR+'">';   
    ...
    if (typeof init_json !== 'undefined' && init_json !== null && init_json['mat_nm_material']) {
        $("#divRecipeArea_"+group_idx+" [id=step" + group_idx + "_" + step + "]").val(init_json['mat_nm_material']);
    } else {
        $("#divRecipeArea_"+group_idx+" [id=step" + group_idx + "_" + step + "]").attr('placeholder','예) '+_MATERIAL_SAMPLE[(step-1)%_MATERIAL_SAMPLE.length]['mat_nm_material']);
    }
    if (typeof init_json !== 'undefined' && init_json !== null && (init_json['mat_no_amount'] || init_json['mat_tx_amount'])) {
			...
			<div class="cont_box pad_l_60">
				...
				<div class="cont_line pad_b_25">
					...
					<textarea name="recipe_content" id="recipe_content"
						...
						placeholder="예) 집에서 쉽게 만드는 뽀모도로 파스타 레시피 입니다" 
						style=" resize: none; margin-left: 90px;">${update.recipe_content}</textarea>
				</div>
				...
				<div class="cont_line">
					<p class="cont_tit4">요리정보</p>
						...
						<select name="recipe_time" id="recipe_time" text="요리시간">
							<option value="5분이내"<c:if test="${recipe_time == '5분이내'}">selected</c:if>>5분이내</option>
							<option value="10분이내"<c:if test="${recipe_time == '10분이내'}">selected</c:if>>10분이내</option>
							<option value="15분이내"<c:if test="${recipe_time == '15분이내'}">selected</c:if>>15분이내</option>
							<option value="20분이내"<c:if test="${recipe_time == '20분이내'}">selected</c:if>>20분이내</option>
							<option value="30분이내"<c:if test="${recipe_time == '30분이내'}">selected</c:if>>30분이내</option>
							<option value="60분이내"<c:if test="${recipe_time == '60분이내'}">selected</c:if>>60분이내</option>
							<option value="90분이내"<c:if test="${recipe_time == '90분이내'}">selected</c:if>>90분이내</option>
							<option value="2시간이내"<c:if test="${recipe_time == '2시간이내'}">selected</c:if>>2시간이내</option>
							<option value="2시간이상"<c:if test="${recipe_time == '2시간이상'}">selected</c:if>>2시간이상</option>
						</select>


						<t1>난이도</t1>						
						<select name="recipe_level" id="recipe_level" text="난이도">
							<option value="Easy"<c:if test="${recipe_time == 'Easy'}">selected</c:if>>Easy</option>
							<option value="Normal"<c:if test="${recipe_time == 'Normal'}">selected</c:if>>Normal</option>
							<option value="Hard"<c:if test="${recipe_time == 'Hard'}">selected</c:if>>Hard</option>
						</select>
							...
							<li id="liMaterialGroup_1">
								<ul id="divMaterialArea_1" class="ui-sortable">
									...
									<!-- <a href="#" class="btn-lineup ui-sortable-handle" data-original-title="" title=""></a> -->
										<c:set var="str1" value="${update.mate_name}"/>
										<c:forEach items="${fn:split(str1, ',') }" var="item">
											<input type="text" name="mate_name" id="mate_name" class="form-control" style="width: 330px;" placeholder="예) 돼지고기" value="${item}">
										</c:forEach>
										<c:set var="str2" value="${update.mate_cnt}"/>
										<c:forEach items="${fn:split(str2, ',') }" var="item">
										<input type="text" name="mate_cnt" id="mate_cnt" class="form-control" style="width: 330px;" placeholder="예) 300g" value="${item}">
										</c:forEach>
									</li>
							...
							<li id="recipeGroup_1">
								...
								<ul id="divRecipeArea_1" class="ui-sortable">
									...
									<li id="rcMaterial_1_1">
										<c:set var="str3" value="${update.step}"/>
										<c:forEach items="${fn:split(str3, ',') }" var="item">
										<input type="text" name="step" id="step" class="form-control" style="width: 650px; height: 150px;" placeholder="step1 &nbsp; &nbsp; 예) 베이컨을 얇게 썰어주세요" value="${item}">										
										</c:forEach>
